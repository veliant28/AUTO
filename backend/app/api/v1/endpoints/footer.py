...
async def get_all_footer(user_id: int = Depends(get_current_user), db: Session = Depends(get_db)):
...
async def update_footer(locale: str, body: FooterUpdate, user_id: int = Depends(get_current_user), db: Session = Depends(get_db)):
    if locale not in LOCALES:
