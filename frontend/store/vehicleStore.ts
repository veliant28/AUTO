...
export const useVehicleStore = create<VehicleState>()(
  persist(
    (set) => ({
      ...
      setMod: (id, name) => set({ modId: id, modName: name, confirmed: false }),
      setVolume: (vol) => set({ volume: vol, engine: null, confirmed: false }),
      setEngine: (eng) => set({ engine: eng, confirmed: false }),
      setCarDetails: (details) =>
        set({
          power: details.power ?? null,
          yearFrom: details.yearFrom ?? null,
          yearTo: details.yearTo ?? null,
        }),
      confirmVehicle: () => set((state) => ({ confirmed: true })),
      clearVehicle: () =>
        set({
          type: null,
          year: null,
