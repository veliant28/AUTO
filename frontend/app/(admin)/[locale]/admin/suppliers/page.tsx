...
function TokenBadge({
  ...
  const { data } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await api.get(
        `/admin/suppliers/${supplier}/token-status`,
      )
      return data as {
        token_status: string
  ...
  React.useEffect(() => {
    if (data?.token_expires_at) {
      ...
      setSecondsLeft(
        Math.max(0, Math.floor((expiresAtRef.current - Date.now()) / 1000)),
      )
    } else {
  ...
  if (secondsLeft !== null && secondsLeft > 0) {
    ...
    return (
      <Badge
        className={`gap-1.5 text-sm font-mono border-0 min-w-[85px] justify-center ${
          isCritical
  ...
  return (
    <Badge className="gap-1.5 text-sm bg-muted text-muted-foreground border-0">
      <Key className="w-3.5 h-3.5" />
      {ta('settings_token_none')}
    </Badge>
  )
}
  ...
  const refreshMutation = useMutation({
    ...
    onSuccess: (result) => {
      if (result.success) {
        ...
        toast.error(
          result.message ||
            ta('settings_refresh_error', { supplier: meta.label }),
        )
      }
    },
    onError: (err: any) => {
  ...
  const authMutation = useMutation({
    ...
    onSuccess: (result) => {
      if (result.success) {
        ...
        toast.error(
          result.message || ta('settings_auth_error', { supplier: meta.label }),
        )
      }
    },
    onError: (err: any) => {
      ...
      <CardHeader>
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2 min-w-0">
            <Badge
              className={`${meta.color} border-0 text-white text-sm shrink-0`}
            >
              {meta.label}
            </Badge>
            <CardTitle className="text-lg truncate">
              {ta('settings_supplier_title', { supplier: meta.label })}
            </CardTitle>
          </div>
          <TokenBadge
            supplier={supplier}
      ...
      <CardHeader>
        <div className="flex items-center gap-2">
          ...
          <CardTitle className="text-lg text-muted-foreground">
            {ta('settings_supplier_placeholder')}
          </CardTitle>
        </div>
      </CardHeader>
      <CardContent className="space-y-4">
...
function ScheduleSection() {
  ...
  const toggleMutation = useMutation({
    mutationFn: async ({
      ...
      const { data } = await api.put(`/admin/schedules/${supplier}`, {
        enabled,
      })
      return data
    },
          ...
          <thead>
            <tr className="border-b bg-muted/50">
              ...
              <th className="text-center p-3 font-medium text-muted-foreground w-[80px]">
                {t('settings_schedule_time')}
              </th>
              <th className="text-left p-3 font-medium text-muted-foreground w-[120px]">
                {t('import_status')}
              </th>
              <th className="text-center p-3 font-medium text-muted-foreground w-[130px]">
                {t('settings_schedule_next')}
              </th>
              <th className="text-left p-3 font-medium text-muted-foreground w-[170px]">
                {t('settings_schedule_last')}
              </th>
          ...
          <tbody>
            {schedules.map((s: any) => (
              <ScheduleRow
                ...
                t={t}
                onToggle={(enabled) =>
                  toggleMutation.mutate({ supplier: s.supplier, enabled })
                }
                onTimeChange={(run_at_time) =>
                  timeMutation.mutate({ supplier: s.supplier, run_at_time })
                }
...
function ScheduleRow({
  ...
  const ssColor =
    scheduleStatusColors[s.schedule_status] || 'bg-gray-400 text-white'
  const statusLabel = scheduleStatusLabel(s.schedule_status, t)

  return (
    <tr className="border-b last:border-0 hover:bg-muted/30">
      <td className="p-3">
        <Badge className={`${color} border-0 text-white text-sm`}>
          {s.supplier}
        </Badge>
      </td>
      ...
      <td className="p-3 text-center">
        {s.enabled ? (
          <ScheduleTimer nextRunUtc={s.next_run_utc} />
        ) : (
          <span className="text-sm text-muted-foreground">—</span>
        )}
      </td>
      <td className="p-3 text-sm text-muted-foreground">
        {formatDate(s.last_run_at, tz)}
      </td>
      <td className="p-3">
        <div className="flex items-center gap-2 pr-2">
          <Progress
            ...
            indicatorClassName={
              s.last_import_status === 'failed'
                ? 'bg-red-500'
                : s.last_import_status === 'complete'
                  ? 'bg-green-500'
                  : 'bg-blue-500'
            }
          />
          <span className="text-sm font-mono text-muted-foreground w-8 text-right">
            {s.last_import_progress || 0}%
          </span>
        </div>
      </td>
      <td className="p-3">
        <div className="flex gap-2 justify-center">
          <Tooltip>
            ...
            <TooltipContent>
              {s.enabled
                ? t('settings_schedule_disabled_short')
                : t('settings_schedule_enabled_short')}
            </TooltipContent>
          </Tooltip>
          <Tooltip>
            <TooltipTrigger asChild>
