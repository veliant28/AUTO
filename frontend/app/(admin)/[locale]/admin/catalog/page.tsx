...
export default function AdminCatalogPage() {
  ...
  const { data: yearsData } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await api.get('/admin/catalog/years', {
  ...
  const { data: makesData } = useQuery({
    ...
    queryFn: async () => {
      if (year == null) return []
      const { data } = await api.get('/admin/catalog/makes', {
        params: { type: vehicleType, year },
      })
  ...
  const { data: modelsData } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data } = await api.get('/admin/catalog/models', {
        params: { type: vehicleType, year, make_id: makeId },
      })
  ...
  const { data: carsData } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data } = await api.get('/admin/catalog/cars', {
        params: { type: vehicleType, year, model_id: modelId },
      })
  ...
  // Filter cars by volume on the frontend
  const filteredCars = React.useMemo(() => {
    if (!volume) return carsData ?? []
    return (carsData ?? []).filter((c) => c.capacity === volume)
  }, [carsData, volume])

  const { data: volumesData } = useQuery({
    queryKey: ['catalog-volumes', vehicleType, year, modelId, carId],
    queryFn: async () => {
      if (year == null || modelId == null) return []
      const params: any = { year, model_id: modelId }
      if (carId != null) params.car_id = carId
      const { data } = await api.get('/admin/catalog/volumes', { params })
  ...
  const { data: enginesData } = useQuery({
    queryKey: ['catalog-engines', vehicleType, year, modelId, volume, carId],
    queryFn: async () => {
      if (year == null || modelId == null) return []
      const params: any = { year, model_id: modelId }
  ...
  const { data: itemsData } = useQuery({
    ...
    enabled: hydrated && !!user && makeId != null,
  })
      ...
      <div className="flex gap-2 flex-wrap items-end">
        ...
        >
          <SelectTrigger className="w-[100px]">
            <SelectValue placeholder={t('catalog_year')} />
          </SelectTrigger>
          <SelectContent>
            {yearsData?.map((y) => (
        ...
        >
          <SelectTrigger className="w-[160px]">
            <SelectValue placeholder={t('catalog_make')} />
          </SelectTrigger>
          <SelectContent>
            {makesData?.map((m) => (
        ...
        >
          <SelectTrigger className="w-[160px]">
            <SelectValue placeholder={t('catalog_model')} />
          </SelectTrigger>
          <SelectContent>
            {modelsData?.map((m) => (
              <SelectItem key={m.id} value={String(m.id)}>
        ...
        >
          <SelectTrigger className="w-[110px]">
            <SelectValue placeholder={t('catalog_volume')} />
          </SelectTrigger>
          <SelectContent>
            {volumesData?.map((v) => (
        ...
        <Select
          key={'mod-' + (filteredCars?.length ?? 'empty')}
          value={carId != null ? String(carId) : ''}
          onValueChange={(v) => {
            const car = filteredCars.find((c) => c.id === Number(v))
            setCarId(Number(v))
            if (car) {
              if (car.capacity) setVolume(car.capacity)
        ...
        >
          <SelectTrigger className="w-[250px]">
            <SelectValue placeholder={t('catalog_modification')} />
          </SelectTrigger>
          <SelectContent>
            {filteredCars?.map((c) => (
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b bg-muted/50">
                    ...
                    {hasEngine && (
                      <th className="text-center p-3 font-medium text-muted-foreground w-[500px]">
                        {t('catalog_engine')}
                      </th>
                    )}
                    {hasEngine && (
                  ...
                  {itemsData?.items?.map((item: any) => (
                    ...
                    >
                      <td className="p-3 text-sm whitespace-nowrap">
                        {(item.constructioninterval || '').replace(
                          /^\d{2}\.(\d{4}) - \d{2}\.(\d{4})$/,
                          '$1 – $2',
                        ) || '—'}
                      </td>
                      <td className="p-3 text-sm font-semibold truncate max-w-[120px]">
                        {item.brand}
                      </td>
                      ...
                      {hasEngine && (
                        <td className="p-3 text-center text-sm font-mono truncate">
                          {item.engine || '—'}
                        </td>
                      )}
                      {hasEngine && (
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-between p-3 border-t">
              <span className="text-sm text-muted-foreground">
                {t('page_of', { page, total: itemsData?.total || 0 })}
              </span>
