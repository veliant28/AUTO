...
export default function ImportPage() {
  ...
  useEffect(() => {
    ...
    return () => {
      delete (window as any).__openImportExport
    }
  }, [])
  ...
  const loadExportParams = async (supplierName: string) => {
    ...
    try {
      ...
      const { data } = await api.get(
        `/admin/imports/export-params/${supplierName}`,
      )
      ...
      <div className="flex gap-4 mb-4 flex-wrap">
        ...
        >
          <SelectTrigger className="w-[140px]">
            <SelectValue placeholder={t('import_status')} />
          </SelectTrigger>
          <SelectContent>
        ...
        >
          <SelectTrigger className="w-[140px]">
            <SelectValue placeholder={supplierLabel} />
          </SelectTrigger>
          <SelectContent>
            <SelectItem value="all">{t('products_filter_all')}</SelectItem>
            ...
            <table className="w-full text-sm table-fixed">
              <thead>
                <tr className="border-b bg-muted/50">
                  ...
                  <th className="text-right p-3 font-medium text-muted-foreground w-[80px]">
                    {t('import_items')}
                  </th>
                  <th className="text-right p-3 font-medium text-muted-foreground w-[80px]">
                    {t('import_size')}
                ...
                {data?.items?.map((item: any) => {
                  ...
                  return (
                    ...
                    >
                      <td className="p-3">
                        <Badge
                          className={`${supplierColors[item.supplier] || 'bg-gray-500 text-white'} border-0 text-sm`}
                        >
                          {item.supplier}
                        </Badge>
                      </td>
                      ...
                      <td className="p-3 text-right text-sm">
                        {item.total_items > 0 ? fmt(item.total_items) : '—'}
                      </td>
                          ...
                          {(item.status === 'complete' ||
                            ...
                            <>
                              ...
                              <Tooltip>
                                <TooltipTrigger asChild>
                                  <a
                                    href={`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000/api/v1'}/admin/imports/${item.id}/download`}
                                    target="_blank"
                                  >
              ...
              <div className="flex justify-center gap-3">
                {['GPL', 'UTR'].map((s) => (
                  <Button
                    ...
                    className={
                      exportSupplier === s
                        ? `${supplierColors[s] || 'bg-gray-500'} text-white hover:opacity-90`
                        : ''
                    }
                      ...
                      <div className="flex flex-wrap gap-1 max-h-40 overflow-y-auto border rounded-lg p-2 bg-background">
                        {exportParams.categories.slice(0, 100).map((c: any) => (
                          <Badge
                            key={c.id}
                            variant={
                              exportCategories.includes(String(c.id))
                                ? 'default'
                                : 'outline'
                            }
                            className="cursor-pointer text-sm"
          ...
          <DialogHeader>
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <DialogDescription>
                  {t('import_delete_confirm_message')}
                </DialogDescription>
              </div>
            </div>
          </DialogHeader>
          {deleteTarget && (
            <div className="rounded-lg bg-muted p-3 text-sm">
              <div className="flex items-center gap-2">
                <Badge
                  className={`${supplierColors[deleteTarget.supplier] || 'bg-gray-500 text-white'} border-0 text-sm`}
                >
                  {deleteTarget.supplier}
                </Badge>
                <span className="text-muted-foreground text-sm">
                  {formatDate(deleteTarget.created_at, tz)}
                </span>
              </div>
            </div>
          )}
