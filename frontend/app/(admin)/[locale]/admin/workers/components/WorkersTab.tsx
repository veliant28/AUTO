...
function RuntimeTimer({ timeStart }: { timeStart: number }) {
  const [runtime, setRuntime] = useState(() =>
    Math.round(Date.now() / 1000 - timeStart),
  )
  useEffect(() => {
...
export default function WorkersTab() {
  ...
  useEffect(() => {
    ...
    setActiveHistory((prev) => {
      const next = [
        ...prev,
        { time: Date.now(), value: data.worker.active_count },
      ]
  ...
  const columns = [
    columnHelper.display({
      ...
      cell: ({ row }) => (
        <div
          className={`w-1.5 h-8 rounded-full ${slotBg(row.original.slot_index)}`}
        />
      ),
    }),
    ...
    columnHelper.accessor('status', {
      ...
      cell: (info) =>
        statusBadge(
          ...
          info.row.original.runtime_seconds,
          info.row.original.slot_index,
        ),
    }),
    columnHelper.accessor('runtime_seconds', {
      ...
      cell: (info) => {
        ...
        if (
          task.import_progress !== null &&
          task.import_progress !== undefined
        ) {
          ...
          return (
            <div className="flex items-center gap-2 min-w-[180px]">
              <Badge
                className={`border-0 gap-1 text-sm font-mono ${isFailed ? 'bg-red-500 text-white' : 'bg-blue-500 text-white'}`}
              >
                <Clock className="w-3.5 h-3.5" />
                {task.time_start ? (
                  <RuntimeTimer timeStart={task.time_start} />
                ) : (
        ...
        return (
          <div className="flex items-center gap-2 min-w-[180px]">
            <Badge
              className={`border-0 gap-1 text-sm font-mono ${isStuck ? 'bg-red-500 text-white' : 'bg-blue-500 text-white'}`}
            >
              <Clock className="w-3.5 h-3.5" />
              {task.time_start ? (
                <RuntimeTimer timeStart={task.time_start} />
        ...
        <Card className="md:col-span-2">
          <CardHeader className="pb-0 flex flex-row items-center justify-between gap-2">
            ...
            <div className="flex items-center gap-1.5 flex-wrap justify-end">
              <Badge
                className={`border-0 gap-1 text-sm ${workerOnline ? 'bg-green-500 text-white' : 'bg-gray-500 text-white'}`}
              >
                <div
                  className={`w-2 h-2 rounded-full ${workerOnline ? 'bg-white' : 'bg-gray-300'}`}
                />
