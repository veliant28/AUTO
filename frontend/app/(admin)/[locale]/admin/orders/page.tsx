...
export default function AdminOrdersPage() {
  ...
  const statusMutation = useMutation({
    ...
    onSuccess: (_data, variables) => {
      ...
      queryClient.invalidateQueries({ queryKey: ['admin-order-history', viewOrderId] });
  ...
  const deleteItemMutation = useMutation({
    mutationFn: async (itemId: number) => {
      const { data } = await api.delete(`/admin/orders/${viewOrderId}/items/${itemId}`);
      return data as AdminOrderDetail;
    },
    onSuccess: (data) => {
  ...
  const openView = async (orderId: number) => {
    setEditMode(false);
  ...
  const handleSave = () => {
    const changes: any = {};
    const itemsChanged = orderDetail?.items.some((item) => editQuantities[item.id] !== item.quantity);
    if (itemsChanged) {
      changes.items = Object.entries(editQuantities).map(([id, qty]) => ({ id: Number(id), quantity: qty }));
    }
    const recipientChanged = orderDetail && (
        ...
        <div className="flex items-center gap-2">
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <Button variant="outline" size="icon" className="h-8 w-8" onClick={() => openView(row.original.id)}>
  ...
  return (
    ...
    <div className="p-6">
      ...
      <Dialog open={!!viewOrderId} onOpenChange={(open) => { if (!open) { setViewOrderId(null); setEditMode(false); setShowHistory(false); } }}>
        <DialogContent className="w-[98vw] max-w-[1800px] h-[90vh] overflow-hidden flex flex-col !p-0 !gap-0" aria-describedby={undefined}>
          {detailLoading ? (
            ...
            <>
              <DialogHeader className="p-6 pb-3 pr-14 flex-shrink-0">
                {showHistory ? (
                  <div className="flex items-center gap-3">
                    <Button variant="ghost" size="icon" className="h-9 w-9 rounded-full" onClick={() => setShowHistory(false)}>
                      <ArrowLeft className="w-4 h-4" />
                    </Button>
                    <DialogTitle className="text-lg">{t('order_history')} #{orderDetail.id}</DialogTitle>
                  </div>
                ) : (
                  <div className="flex items-start justify-between">
                    <div className="space-y-1">
                      <div className="flex items-center gap-3">
                        <DialogTitle className="text-lg">{t('order_details')} #{orderDetail.id}</DialogTitle>
                      ...
                      <p className="text-sm text-muted-foreground">{formatDate(orderDetail.created_at)}</p>
                    </div>
                    <div className="flex items-center gap-3">
                      <Select
                        value={orderDetail.status}
                        onValueChange={(val) => statusMutation.mutate({ orderId: orderDetail.id, status: val })}
                      >
                    ...
                    {(!historyLogs || historyLogs.length === 0) ? (
                      ...
                      historyLogs.map((log, idx) => (
                        <div key={log.id} className="relative pb-6">
                          <div className={`absolute -left-[34px] top-1.5 w-5 h-5 rounded-full border-[3px] border-background ${idx === 0 ? 'bg-green-500' : 'bg-blue-500'}`} />
                          <div className="flex items-center gap-2 mb-1 flex-wrap">
                            {log.user_group && (
                              <Badge className={`${ROLE_BADGE_COLORS[log.user_group] || 'bg-orange-500 text-white'} border-0 text-sm`}>
                                {t(log.user_group)}
                              </Badge>
                            )}
                            <span className="font-medium">{log.user_name || '—'}</span>
                            <span className="text-sm text-muted-foreground">
                              {new Date(log.created_at + 'Z').toLocaleString(localeKey, {
                                day: 'numeric', month: 'long', year: 'numeric', hour: '2-digit', minute: '2-digit',
                              })}
                            </span>
                          </div>
                          <p className="text-muted-foreground pl-1">
                            {log.action === 'status_change' ? (
                              <span className="flex items-center gap-1">
                                ...
                                {(() => {
                                  const m = log.details?.match(/статус:\s*(\w+)\s*→\s*(\w+)/);
                                  if (m) return <span>{t('status_changed')}: {t('order_' + m[1])} → {t('order_' + m[2])}</span>;
                                  return log.details;
                                })()}
                              </span>
                            ) : (
                              formatPhonesInText(log.details || log.action).replace(/\b(warehouse|parcel_locker|courier)\b/g, (m) => t(m))
                            )}
                          </p>
                        </div>
                      ))
                      ...
                      <div className="space-y-3 flex-1 overflow-y-auto pr-1 mt-3">
                        {orderDetail.items.map((item) => {
                          ...
                          <div key={item.id} className="flex gap-3 p-3 rounded-lg border bg-card transition-colors">
                            <div className={`aspect-square w-[80px] shrink-0 rounded-lg overflow-hidden relative flex items-center justify-center bg-gradient-to-br ${getBrandColor(item.brand)}`}>
                            ...
                            <div className="flex-1 min-w-0 space-y-1">
                              <div className="flex items-start justify-between gap-2">
                                ...
                                <div className="flex items-center shrink-0 gap-2">
                                  {item.sku && (
                                    <Badge className="bg-blue-500 text-white border-0 text-sm">{item.sku}</Badge>
                                  )}
                                  {editMode && (
                                    <Tooltip>
                                      <TooltipTrigger asChild>
                      ...
                      <div className="flex-1 space-y-2 text-sm overflow-y-auto mt-3">
                        ...
                        {['last_name', 'first_name', 'middle_name'].map((field) => (
                          <div key={field} className="flex justify-between items-center">
                            ...
                            {editMode ? (
                              ...
                              <span>{(orderDetail as any)[field] || '—'}</span>
                            )}
                          </div>
                        ))}
                        <Separator />
                        ...
                        <RadioGroup value={editRecipient.delivery_type} onValueChange={(v) => setEditRecipient((p) => ({ ...p, delivery_type: v }))}
                          ...
                          <Tooltip>
                            <TooltipTrigger asChild>
                              <div className="flex items-center justify-center gap-2 rounded-lg border p-3 has-data-[state=checked]:border-primary cursor-pointer">
                                <RadioGroupItem value="parcel_locker" id="dpl" className="cursor-pointer" />
                                <Label htmlFor="dpl" className="cursor-pointer"><Container className="w-5 h-5" /></Label>
                              </div>
                            </TooltipTrigger>
                            <TooltipContent>{t('parcel_locker')}</TooltipContent>
                          </Tooltip>
                          <Tooltip>
                            <TooltipTrigger asChild>
                              <div className="flex items-center justify-center gap-2 rounded-lg border p-3 has-data-[state=checked]:border-primary cursor-pointer">
                        ...
                        {['delivery_city', 'delivery_warehouse'].map((field) => (
                          <div key={field} className="space-y-1">
                            <span className="text-muted-foreground">{t(field)}</span>
                            {editMode ? (
                              <Input className="h-9 w-full text-sm" value={editRecipient[field] || ''} maxLength={200}
                                onChange={(e) => setEditRecipient((p) => ({ ...p, [field]: e.target.value }))} />
                            ) : (
                              <p className="text-sm">{(orderDetail as any)[field] || '—'}</p>
                            )}
                          </div>
                        ))}
                      </div>
              ...
              <div className="flex-shrink-0 p-4 pt-3">
                {showHistory ? <div className="h-9" /> : editMode ? (
                  <div className="flex gap-2">
                    <Button variant="outline" className="h-9" onClick={() => { setEditMode(false); }}>
                      {t('cancel')}
                    </Button>
                      <Button className="h-9 gap-2" onClick={handleSave} disabled={updateMutation.isPending}>
                  ...
                  <div className="flex gap-2">
                    <Button variant="outline" className="gap-1.5 h-9" onClick={() => setShowHistory(true)}>
                      <History className="w-4 h-4" /> {t('order_history')}
                    </Button>
                    <Button variant="outline" className="gap-1.5 h-9" onClick={enterEditMode}>
