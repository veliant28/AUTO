...
export default function CategoryNav() {
  ...
  const { data } = useQuery({
    ...
    queryFn: async () => {
      ...
      return data as {
        ...
        zapchasti_dlya_to: TosSection[] | null
      }
    },
    staleTime: 300_000,
  })
  ...
  const activeIdx =
    active !== null &&
    active < categories.length + (toSections.length > 0 ? 1 : 0)
      ? active
      : null
  ...
  if (!hydrated) {
    return <div className="hidden lg:flex flex-1 mx-4 h-full" />
  }

  return (
    <div
      ref={navRef}
      className="hidden lg:flex flex-1 mx-4 h-full items-center justify-around gap-1"
    ...
    >
      ...
      {toSections.length > 0 && (
        <button
          ref={(el) => {
            btnRefs.current[categories.length] = el
          }}
      ...
      {activePopup && popupTop !== null && (
        <div
          ...
          onMouseLeave={handleMouseLeave}
          className="fixed z-50 left-0 right-0 bg-popover border-b rounded-b-lg shadow-xl py-6 max-h-[70vh] overflow-y-auto animate-nav-in"
          style={{ top: popupTop }}
        >
          <div className="container mx-auto px-4">{activePopup}</div>
        </div>
      )}
    </div>
  )
