...
export function useVehicleYears(type: VehicleType) {
  return useQuery({
    ...
    queryFn: async () => {
      const { data } = await api.get('/catalog/vehicle/years', {
        params: { type },
      })
...
export function useVehicleMakes(type: VehicleType, year: string | null) {
  return useQuery({
    ...
    queryFn: async () => {
      const { data } = await api.get('/catalog/vehicle/makes', {
        params: { type, year },
      })
...
export function useVehicleModels(
  ...
  return useQuery({
    ...
    queryFn: async () => {
      const { data } = await api.get('/catalog/vehicle/models', {
        params: { type, year, make_id: makeId },
      })
      return data as { id: number; name: string }[]
    },
...
export function useVehicleCars(
  type: VehicleType,
  year: string | null,
  modelId: string | null,
) {
  return useQuery({
    ...
    queryFn: async () => {
      const { data } = await api.get('/catalog/vehicle/cars', {
        params: { type, year, model_id: modelId },
      })
      return data as {
        id: number
        name: string
...
export function useVehicleEngines(
  year: string | null,
  carId: string | null,
  volume: string | null,
) {
  return useQuery({
    queryKey: ['vehicle-engines', year, carId, volume],
    queryFn: async () => {
      const params: any = { year, car_id: carId }
